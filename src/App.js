...
// ============================================================
// ✨ LoginScreen — 회사 이메일 + 인증코드 로그인/가입 화면
// 1단계: 이메일 입력 → 인증코드 발송
// 2단계: 코드 입력 (+ 최초 가입이면 이름도 함께 입력) → 로그인 완료
// ============================================================
function LoginScreen({ onLoginSuccess, showToast }) {
  ...
  const handleRequestCode = async (e) => {
    ...
    try {
      setLoading(true);
      setErrorMsg('');
      const res = await axios.post(`${BASE_URL}/auth/request-code`, { email: email.trim() });
      if (res.data.success) {
        setIsNewUser(!!res.data.isNewUser);
        setStep('code');
        showToast('인증코드를 이메일로 보냈습니다.', 'success');
      }
    } catch (err) {
      setErrorMsg(err.response?.data?.message || '인증코드 발송에 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyCode = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.requiresName) {
        setIsNewUser(true);
        setErrorMsg('최초 로그인입니다. 이름을 입력해 주세요.');
      } else {
    ...
    <div className="login-screen">
      <div className="login-card">
        <div className="login-logo">
          <img src="/logo.png" alt="회사 로고" width="32" height="32" />
        ...
        {step === 'code' && (
          <form onSubmit={handleVerifyCode} className="login-form">
            <p className="login-sent-to">📧 <strong>{email}</strong>로 인증코드를 보냈습니다.</p>
...
function App() {
  ...
  // ============================================================
  // 로그아웃 — 서버 세션을 폐기하고 토큰/이름을 지운 뒤 로그인 화면으로 되돌림
  // ============================================================
  const handleLogout = () => {
    if (window.confirm(`${userName}님, 로그아웃하시겠습니까?`)) {
      axios.post(`${BASE_URL}/auth/logout`).catch(() => {}); // 실패해도 클라이언트 쪽은 그대로 로그아웃 처리
  ...
  // ✨ 알림 체크 (앱 시작 및 주기적)
   useEffect(() => {
    // 데이터 로딩은 로그인 여부와 무관하게 먼저 시작 — 로그인 완료 시점엔 이미
    // 응답이 와 있거나 곧 도착하도록 해 체감 로딩 시간을 줄인다.
    loadCategories();
    loadAlerts();

    // ✨ 이전엔 prompt()로 사번을 입력받아 프론트에서만 "인증"을 흉내냈으나(위조 가능),
    //    이제는 서버가 발급한 세션 토큰을 localStorage에서 복원하는 방식으로 바뀌었다.
    //    토큰이 없거나 만료됐으면 로그인 화면(LoginScreen)이 대신 렌더링된다.
    ...
    // ✨ 세션이 만료되거나 무효화되면(서버가 401 반환) 자동으로 로그아웃 처리하고
    //    로그인 화면으로 되돌린다 — 사용자가 계속 실패하는 요청과 마주치지 않도록.
    ...
    // ✨ 저장된 토큰이 있으면 앱을 열 때마다 새 30일짜리로 갱신.
    //    만료됐으면 401 → 위 인터셉터가 로그인 화면으로 보냄 (작업 도중이 아니라 앱 시작 시점에).
    //    콜드스타트 등 네트워크 오류는 무시 — 로그아웃시키지 않고 다음 실행 때 다시 갱신.
    if (savedToken && savedName) {
      axios.post(`${BASE_URL}/auth/refresh`)
        .then(res => handleLoginSuccess(res.data.token, res.data.name))
        .catch(() => {});
    }
  ...
  // 로그인 성공 시 LoginScreen에서 호출 — 토큰 저장 + axios 기본 헤더 설정
      ...
      <header className="top-nav">
        <div className="nav-left">
          <button className="nav-logo" onClick={() => { setPage('main'); setSearchResults([]); setIsSearching(false); }}>
            <img src="/logo.png" alt="회사 로고" width="24" height="24" />
            <span>Smart Inventory</span>
          </button>
          {/* ✨ 3. 현재 로그인 사용자 표시 */}
...
function MainPage({ onSheetClick, onSparePartClick, onUsageSummaryClick, facilityLists, inventoryData, userName, showToast, onDataRefresh, onSummaryClick, alerts, onSearch, searchResults, isSearching, onSearchResultClick }) {
  ...
  const [showFacilityTypeSelect, setShowFacilityTypeSelect] = useState(false); // ✨ "설비별" 카드 클릭 시 충전/타정/제조/유틸리티 선택 화면
  const [showPartActionSelect, setShowPartActionSelect] = useState(false); // ✨ "부품추가・삭제" 카드 클릭 시 추가/삭제 선택 화면
  ...
  return (
    <div className="main-page">
      ...
      {/* ✨ 메인 대분류 카드 4개: 설비별 / 부품별 / 사용내역 요약 / 부품추가・삭제 */}
      <div className="category-grid main-four-grid">
        <button className="category-card" onClick={() => setShowFacilityTypeSelect(true)} style={{ borderColor: '#f97316' }}>
          <div className="category-icon-wrap">🏭</div>
          <div className="category-label">설비별</div>
        </button>
        ...
        >
          <div className="category-icon-wrap">📦</div>
          <div className="category-label">부품별</div>
        </button>
        ...
        <button
          className="category-card"
          onClick={() => setShowPartActionSelect(true)}
          style={{ borderColor: '#16a34a' }}
        >
          <div className="category-icon-wrap">➕</div>
          <div className="category-label">부품추가・삭제</div>
        </button>
      </div>

      {/* ✨ "설비별" 선택 화면 — 충전/타정/제조/유틸리티 중 하나를 고르면 해당 설비 목록으로 이동 */}
      {showFacilityTypeSelect && (
        <div className="modal-overlay" onClick={() => setShowFacilityTypeSelect(false)}>
          <div className="facility-type-select-modal" onClick={e => e.stopPropagation()}>
            <div className="part-manage-modal-header">
              <h3>🏭 설비 선택</h3>
              <button className="modal-close-btn" onClick={() => setShowFacilityTypeSelect(false)}>✕</button>
            </div>
            <div className="category-grid">
              {processSheets.map((sheet) => (
                <button
                  key={sheet}
                  className="category-card"
                  onClick={() => { setShowFacilityTypeSelect(false); onSheetClick(sheet); }}
                >
      ...
      {/* ✨ "부품추가・삭제" 선택 화면 — 추가할지 삭제할지 고르면 기존 PartManageModal로 이어짐 */}
      {showPartActionSelect && (
        <div className="modal-overlay" onClick={() => setShowPartActionSelect(false)}>
          <div className="facility-type-select-modal" onClick={e => e.stopPropagation()}>
            <div className="part-manage-modal-header">
              <h3>➕ 부품추가・삭제</h3>
              <button className="modal-close-btn" onClick={() => setShowPartActionSelect(false)}>✕</button>
            </div>
            <div className="part-action-select-grid">
              <button
                className="part-action-select-btn add"
                onClick={() => { setShowPartActionSelect(false); setPartModalMode('add'); }}
              >
                <span className="part-manage-icon">＋</span>
                <span>부품추가</span>
              </button>
              <button
                className="part-action-select-btn delete"
                onClick={() => { setShowPartActionSelect(false); setPartModalMode('delete'); }}
              >
                <span className="part-manage-icon">－</span>
                <span>부품삭제</span>
              </button>
  ...
  {isCommonSheet ? (
    ...
    <h2 className="main-cat-title">{categoryName}</h2>
      ...
      <div className="detail-list">
        {[...items]
          .sort((a, b) => String(a.모델명 || '').localeCompare(String(b.모델명 || ''), 'ko', { numeric: true, sensitivity: 'base' }))
          .map((item) => {
  ...
  <div className="detail-model-wrapper">
    ...
    {!isEditing && (
              ...
              <div className="detail-card-body">
                {/* ✨ 압축형 2x2 정보 그리드 — 텍스트가 긴 값(예: 보관위치 20자 이상)은
                    wide-cell 클래스로 그 항목만 자동으로 한 줄 전체를 차지하게 해 줄바꿈 여유를 줌 */}
                <div className="detail-info-grid">
                  ...
                  <div className={`detail-info-cell ${String(item.최종수정시각 || '').length > 16 ? 'wide-cell' : ''}`}>
                  ...
                  <div className={`detail-info-cell ${String(item.보관장소 || '').length > 10 ? 'wide-cell' : ''}`}>
...
function LogsPage({ onBack, inventoryData, facilityLists }) {
  const LIMIT = 50;
  // ✨ 입고/출고/기타를 하나의 통합 목록에서 잘라 쓰지 않고, 각각 독립적으로 조회한다.
  //    (이전 방식: 최근 50건을 통째로 가져와 화면에서 입고/출고로 나눴는데, 입고가 압도적으로
  //     많이 쌓이면 출고 이력이 그 50건 안에 아예 안 들어와 "내역 없음"으로 보이는 문제가 있었음)
  const makeInitialState = () => ({ logs: [], offset: 0, total: 0, loadingMore: false });
  const [inState, setInState] = useState(makeInitialState());
  const [outState, setOutState] = useState(makeInitialState());
  const [etcState, setEtcState] = useState(makeInitialState());
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState(''); // ✨ 설비명/모델명/부품종류 통합 검색어

  async function fetchColumn(actionParam, offsetVal, q) {
    const params = new URLSearchParams({ limit: LIMIT, offset: offsetVal });
    if (actionParam.action) params.append('action', actionParam.action);
    if (actionParam.excludeActions) params.append('excludeActions', actionParam.excludeActions);
    if (q && q.trim()) params.append('q', q.trim());
    const res = await axios.get(`${BASE_URL}/inventory/logs?${params}`);
    return res.data;
  }

  async function loadAll(q = searchQuery) {
    try {
      setLoading(true);
      const [inRes, outRes, etcRes] = await Promise.all([
        fetchColumn({ action: '입고' }, 0, q),
        fetchColumn({ action: '출고' }, 0, q),
        fetchColumn({ excludeActions: '입고,출고' }, 0, q)
      ]);
      setInState({ logs: inRes.data, offset: inRes.data.length, total: inRes.total, loadingMore: false });
      setOutState({ logs: outRes.data, offset: outRes.data.length, total: outRes.total, loadingMore: false });
      setEtcState({ logs: etcRes.data, offset: etcRes.data.length, total: etcRes.total, loadingMore: false });
    } catch (err) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => { loadAll(searchQuery); }, 300);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchQuery]);

  async function handleLoadMore(type) {
    const stateMap = { in: [inState, setInState, { action: '입고' }], out: [outState, setOutState, { action: '출고' }], etc: [etcState, setEtcState, { excludeActions: '입고,출고' }] };
    const [state, setState, actionParam] = stateMap[type];
    try {
      setState(prev => ({ ...prev, loadingMore: true }));
      const res = await fetchColumn(actionParam, state.offset, searchQuery);
      setState(prev => ({
        logs: [...prev.logs, ...res.data],
        offset: prev.offset + res.data.length,
        total: res.total,
        loadingMore: false
      }));
    } catch (err) {
  ...
  const inLogs = inState.logs;
  const outLogs = outState.logs;
  const etcLogs = etcState.logs;
  const totalAll = inState.total + outState.total + etcState.total;
  ...
  return (
    <div className="logs-page">
      ...
      <div className="logs-filter-bar">
        ...
        <span className="logs-total-count">총 {totalAll}건</span>
      </div>

      {/* 입고 / 출고 2컬럼 — 각각 독립적으로 조회되므로 한쪽이 밀려 사라지지 않음 */}
      <div className="logs-split-wrap">
        {/* 입고 컬럼 */}
        <div className="logs-col logs-col-in">
          <div className="logs-col-header logs-col-header-in">
            📥 입고 <span className="logs-col-count">{inState.total}건</span>
          </div>
          <div className="logs-col-body">
            ...
            {inLogs.length < inState.total && (
              <button className="logs-load-more-col" onClick={() => handleLoadMore('in')} disabled={inState.loadingMore}>
                {inState.loadingMore ? '로드 중...' : `더보기 (${inState.total - inLogs.length}건 남음)`}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="logs-col logs-col-out">
          <div className="logs-col-header logs-col-header-out">
            📤 출고 <span className="logs-col-count">{outState.total}건</span>
          </div>
          <div className="logs-col-body">
            {outLogs.length === 0
            ...
            {outLogs.length < outState.total && (
              <button className="logs-load-more-col" onClick={() => handleLoadMore('out')} disabled={outState.loadingMore}>
                {outState.loadingMore ? '로드 중...' : `더보기 (${outState.total - outLogs.length}건 남음)`}
              </button>
            )}
          </div>
        </div>
      ...
      {etcState.total > 0 && (
        <div className="logs-etc-section">
          <div className="logs-col-header logs-col-header-etc">
            ✏️ 수량변경 <span className="logs-col-count">{etcState.total}건</span>
          </div>
          {etcLogs.map(log => (
          ...
          {etcLogs.length < etcState.total && (
            <button className="logs-load-more-col" onClick={() => handleLoadMore('etc')} disabled={etcState.loadingMore}>
              {etcState.loadingMore ? '로드 중...' : `더보기 (${etcState.total - etcLogs.length}건 남음)`}
            </button>
          )}
        </div>
      )}
...
function AIChatBar({ onInventoryUpdate, showToast }) {
  ...
  // 브라우저 내장 음성인식 (Chrome/Edge 지원, Safari/Firefox는 미지원일 수 있음)
  function handleMicClick() {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;

    if (!SpeechRecognition) {
      showToast && showToast('이 브라우저는 음성인식을 지원하지 않아요. Chrome을 사용해보세요.');
      return;
    }
    ...
    recognition.onerror = (event) => {
      if (event.error === 'not-allowed' || event.error === 'permission-denied') {
        showToast && showToast('마이크 권한을 허용해주세요.');
      } else if (event.error !== 'no-speech' && event.error !== 'aborted') {
        showToast && showToast('음성인식 중 오류가 발생했어요.');
      }
    };
